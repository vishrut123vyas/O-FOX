...
from device_simulator import CPUSensor, RAMMonitor, DiskScanner, SensorHub

# Initialize controller and demo agents
controller = QFoxController()

# Flag to prevent multiple device initializations
devices_initialized = False

def initialize_devices():
    """Initialize device simulation only once."""
    global devices_initialized
    if not devices_initialized:
        controller.create_demo_devices()
        controller.start_device_simulation()
        devices_initialized = True

# Initialize device simulator instances
cpu_sensor = CPUSensor()
ram_monitor = RAMMonitor()
disk_scanner = DiskScanner()
sensor_hub = SensorHub()

# Device access log for tracking agent interactions
device_access_log = []
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div(className="nav-dot", id="nav-dot-6")
        ...
        html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        html.Div([
                         ...
                         html.Div([
                             html.H3("Device & Sensor Network", className="card-title"),
                             html.Div([
                                 html.Button("Add Demo Devices", id="add-demo-devices-btn", className="glow-button accent"),
                                 html.Button("Connect New Device", id="connect-device-btn", className="glow-button primary")
                             ], className="button-group"),
                             html.Div(id="devices-table-container", className="device-table-container")
                         ], className="glass-card wide"),
                         
                         html.Div([
                             html.H3("Device Status Overview", className="card-title"),
                             html.Div(id="device-status-container", className="device-status-container")
                         ], className="glass-card wide")
            ...
            # Slide 6: Device & Sensor Monitor
            html.Div([
                html.Div([
                    html.H2("Device & Sensor Monitor", className="slide-title"),
                    
                    html.Div([
                        # Real-time metric cards
                        html.Div([
                            html.Div([
                                html.H3("CPU Metrics", className="card-title"),
                                html.Div([
                                    html.Div([
                                        html.Span(id="cpu-temp", className="metric-value"),
                                        html.Span("Temperature (°C)", className="metric-label")
                                    ], className="metric-item"),
                                    html.Div([
                                        html.Span(id="cpu-usage", className="metric-value"),
                                        html.Span("Usage (%)", className="metric-label")
                                    ], className="metric-item")
                                ], className="metrics-grid")
                            ], className="glass-card"),
                            
                            html.Div([
                                html.H3("RAM Metrics", className="card-title"),
                                html.Div([
                                    html.Div([
                                        html.Span(id="ram-used", className="metric-value"),
                                        html.Span("Used (GB)", className="metric-label")
                                    ], className="metric-item"),
                                    html.Div([
                                        html.Span(id="ram-available", className="metric-value"),
                                        html.Span("Available (GB)", className="metric-label")
                                    ], className="metric-item")
                                ], className="metrics-grid")
                            ], className="glass-card"),
                            
                            html.Div([
                                html.H3("Disk Metrics", className="card-title"),
                                html.Div([
                                    html.Div([
                                        html.Span(id="disk-usage", className="metric-value"),
                                        html.Span("Usage (%)", className="metric-label")
                                    ], className="metric-item"),
                                    html.Div([
                                        html.Span(id="disk-speed", className="metric-value"),
                                        html.Span("Read Speed (MB/s)", className="metric-label")
                                    ], className="metric-item")
                                ], className="metrics-grid")
                            ], className="glass-card"),
                            
                            html.Div([
                                html.H3("Environment", className="card-title"),
                                html.Div([
                                    html.Div([
                                        html.Span(id="env-temp", className="metric-value"),
                                        html.Span("Temperature (°C)", className="metric-label")
                                    ], className="metric-item"),
                                    html.Div([
                                        html.Span(id="env-humidity", className="metric-value"),
                        ...
                        html.Div([
                            html.Div([
                                html.H3("CPU Usage Gauge", className="card-title"),
                                dcc.Graph(id="cpu-gauge", className="gauge-container")
                            ], className="glass-card"),
                            
                            html.Div([
                                html.H3("RAM Usage Gauge", className="card-title"),
                                dcc.Graph(id="ram-gauge", className="gauge-container")
                            ], className="glass-card")
                        ], className="gauges-grid"),
                        
                        # Environmental readings table
                        html.Div([
                            html.H3("Environmental Sensor Data", className="card-title"),
                            dash_table.DataTable(
                                id="environmental-table",
                                columns=[
                                    {"name": "Sensor", "id": "sensor"},
                                    {"name": "Temperature (°C)", "id": "temperature"},
                                    {"name": "Humidity (%)", "id": "humidity"},
                                    {"name": "Motion Detected", "id": "motion"},
                        ...
                        # Agent access log
                        html.Div([
                            html.H3("Agent Device Access Log", className="card-title"),
                            html.Div(id="agent-access-log", className="access-log-container")
                        ], className="glass-card wide")
                    ], className="device-monitor-section")
                ], className="slide-content")
            ], className="slide", id="slide-6")
...
def update_system_overview(n):
    try:
        ...
        devices = controller.get_devices_status()
        device_efficiency = 0.0
        if devices:
            online_devices = sum(1 for d in devices if d.get('status') == 'online')
            device_efficiency = online_devices / len(devices)
        
        # Combine agent and device efficiency
        agent_efficiency = controller.system_metrics.get('system_efficiency', 0.0)
        total_efficiency = (agent_efficiency + device_efficiency) / 2
        
        return total_agents, total_tasks, f"{total_efficiency:.1%}"
    except Exception as e:
        print(f"Error in update_system_overview: {e}")
        return 0, 0, "0%"
...
@app.callback(
    Output("add-demo-devices-btn", "n_clicks"),
    [Input("add-demo-devices-btn", "n_clicks")],
    prevent_initial_call=True
)
def add_demo_devices(n_clicks):
    if n_clicks:
        controller.create_demo_devices()
    return 0
...
def update_devices_table(n):
    try:
        ...
        if not devices:
            return html.Div("No devices connected", className="no-devices-message")
        
        # Create table data
        table_data = []
        for device in devices:
            last_data = device.get('last_data', {})
            last_data_str = "No data" if not last_data else f"{last_data.get('value', 'N/A')} {last_data.get('unit', '')}"
            
            table_data.append({
                'ID': device.get('device_id', 'N/A')[:8] + "...",
                'Name': device.get('name', 'Unknown'),
                'Type': device.get('device_type', 'Unknown').replace('_', ' ').title(),
                'Status': device.get('status', 'Unknown').title(),
                'Battery': f"{device.get('battery_level', 0):.1f}%",
                'Signal': f"{device.get('signal_strength', 0):.1f}%",
                'Last Data': last_data_str,
                'Assigned Task': device.get('assigned_task') or "None"
            })
        ...
        return dash_table.DataTable(
            ...
            columns=[
                ...
                {"name": "Last Data", "id": "Last Data"},
                {"name": "Assigned Task", "id": "Assigned Task"}
            ],
            data=table_data,
            style_cell={
                "textAlign": "center",
        ...
        print(f"Error in update_devices_table: {e}")
        return html.Div("Error loading devices table", className="error-message")
...
def update_device_status(n):
    try:
        ...
        if not devices:
            return html.Div("No devices to display", className="no-devices-message")
        ...
        for device in devices:
            ...
            total_battery += device.get('battery_level', 0)
            total_signal += device.get('signal_strength', 0)
        
        avg_battery = total_battery / len(devices) if devices else 0
        avg_signal = total_signal / len(devices) if devices else 0
        
        return html.Div([
            html.Div([
                html.Div([
                    html.Span(f"{len(devices)}", className="metric-value"),
                    html.Span("Total Devices", className="metric-label")
                ], className="metric-item"),
                html.Div([
                    html.Span(f"{status_counts.get('online', 0)}", className="metric-value"),
                    html.Span("Online", className="metric-label")
                ], className="metric-item"),
                html.Div([
                    html.Span(f"{status_counts.get('busy', 0)}", className="metric-value"),
                    html.Span("Busy", className="metric-label")
                ], className="metric-item"),
                html.Div([
                    html.Span(f"{avg_battery:.1f}%", className="metric-value"),
                    html.Span("Avg Battery", className="metric-label")
                ], className="metric-item"),
                html.Div([
                    html.Span(f"{avg_signal:.1f}%", className="metric-value"),
                    html.Span("Avg Signal", className="metric-label")
                ], className="metric-item")
            ], className="metrics-grid")
        ])
    except Exception as e:
        print(f"Error in update_device_status: {e}")
        return html.Div("Error loading device status", className="error-message")
...
@app.callback(
    [Output("cpu-temp", "children"),
     ...
     Output("disk-speed", "children"),
     Output("env-temp", "children"),
     Output("env-humidity", "children")],
    [Input("update-interval", "n_intervals")]
)
def update_device_metrics(n):
    try:
        # Get data from device simulators
        cpu_data = cpu_sensor.get_data()
        ram_data = ram_monitor.get_data()
        disk_data = disk_scanner.get_data()
        env_data = sensor_hub.get_data()
        
        # Log agent access (simulate agent interactions)
        if n and n % 5 == 0:  # Every 15 seconds (5 * 3 second intervals)
            agents = list(controller.agents.values())
            if agents:
                random_agent = random.choice(agents)
                device_access_log.append({
                    'timestamp': datetime.now().strftime('%H:%M:%S'),
                    'agent': random_agent.name,
                    'device': random.choice(['CPU Sensor', 'RAM Monitor', 'Disk Scanner', 'Sensor Hub']),
                    'action': 'Data Access'
                })
                # Keep only last 10 entries
                if len(device_access_log) > 10:
                    device_access_log.pop(0)
        
        return (
            f"{cpu_data['temperature']:.1f}°C",
            f"{cpu_data['usage_percent']:.1f}%",
            f"{ram_data['used_gb']:.1f} GB",
            f"{ram_data['available_gb']:.1f} GB",
            f"{disk_data['disk_used_percent']:.1f}%",
            f"{disk_data['disk_read_speed']:.0f} MB/s",
            f"{env_data['temperature']:.1f}°C",
            f"{env_data['humidity']:.1f}%"
        )
    except Exception as e:
...
def update_gauges(n):
    try:
        ...
        total_ram = ram_data['used_gb'] + ram_data['available_gb']
        ram_usage_percent = (ram_data['used_gb'] / total_ram * 100) if total_ram > 0 else 0
        
        # CPU gauge with color coding
        cpu_color = "#00ffab" if cpu_data['temperature'] < 60 else "#ffa726" if cpu_data['temperature'] < 80 else "#ff6b6b"
        ...
        # RAM gauge with color coding
        ram_color = "#00ffab" if ram_usage_percent < 50 else "#ffa726" if ram_usage_percent < 80 else "#ff6b6b"
        
        ram_gauge = go.Figure(go.Indicator(
            mode="gauge+number+delta",
            value=ram_usage_percent,
            domain={'x': [0, 1], 'y': [0, 1]},
            title={'text': "RAM Usage (%)", 'font': {'color': 'white'}},
            delta={'reference': 50},
            gauge={
                'axis': {'range': [None, 100], 'tickcolor': 'white', 'tickfont': {'color': 'white'}},
                'bar': {'color': ram_color},
        ...
        # Update layout for both gauges
        for fig in [cpu_gauge, ram_gauge]:
            fig.update_layout(
                height=300,
                paper_bgcolor='rgba(0,0,0,0)',
                plot_bgcolor='rgba(0,0,0,0)',
...
@app.callback(
    [Output("environmental-table", "data"),
     Output("device-status-table", "data"),
     Output("agent-access-log", "children")],
    [Input("update-interval", "n_intervals")]
)
def update_device_tables(n):
    try:
        # Environmental table data
        env_data = sensor_hub.get_data()
        environmental_data = [{
            'sensor': 'Sensor Hub',
            'temperature': f"{env_data['temperature']:.1f}°C",
            'humidity': f"{env_data['humidity']:.1f}%",
            'motion': "Yes" if env_data['motion_detected'] else "No",
            'last_update': datetime.now().strftime('%H:%M:%S')
        }]
        
        # Device status table data
        devices = [
            {'device': 'CPU Sensor', 'status': cpu_sensor.status, 'last_activity': datetime.now().strftime('%H:%M:%S'), 'data_points': 'Continuous'},
            {'device': 'RAM Monitor', 'status': ram_monitor.status, 'last_activity': datetime.now().strftime('%H:%M:%S'), 'data_points': 'Continuous'},
            {'device': 'Disk Scanner', 'status': disk_scanner.status, 'last_activity': datetime.now().strftime('%H:%M:%S'), 'data_points': 'Continuous'},
            {'device': 'Sensor Hub', 'status': sensor_hub.status, 'last_activity': datetime.now().strftime('%H:%M:%S'), 'data_points': 'Continuous'}
        ]
        
        # Agent access log
        log_entries = []
        for entry in device_access_log[-5:]:  # Show last 5 entries
            log_entries.append(
                html.Div([
                    html.Span(f"[{entry['timestamp']}] ", className="log-timestamp"),
                    html.Span(f"{entry['agent']} ", className="log-agent"),
                    html.Span(f"accessed {entry['device']} - {entry['action']}", className="log-action")
                ], className="log-entry")
            )
        
        if not log_entries:
            log_entries.append(
                html.Div("No recent device access activity", className="no-log-entries")
            )
        
        return environmental_data, devices, log_entries
        
    except Exception as e:
        print(f"Error updating device tables: {e}")
        return [], [], []

# Initialize devices when dashboard loads
@app.callback(
    Output("update-interval", "n_intervals"),
    [Input("update-interval", "n_intervals")],
    prevent_initial_call=False
)
def initialize_dashboard(n):
    """Initialize devices when dashboard first loads."""
    if n is None or n == 0:
        initialize_devices()
    return n
...
@app.callback(
    [Output(f"slide-{i}", "className") for i in range(1, 7)] +
    [Output(f"nav-dot-{i}", "className") for i in range(1, 7)] +
    [Output("current-slide", "data")],
    [Input(f"nav-dot-{i}", "n_clicks") for i in range(1, 7)] +
...
def navigate_slides(*args):
    ...
    if not ctx.triggered:
        return ["slide active"] + ["slide"] * 5 + ["nav-dot active"] + ["nav-dot"] * 5 + [1]
